-- Owner-only, archived deletion of a complete CRM customer chain.
-- Apply after owner_quote_deletion_override_migration.sql and client_crm_profile_notes_migration.sql.
...
BEGIN
  ...
  WHERE client_id = ANY(contact_ids) OR opportunity_id = ANY(opportunity_ids) OR site_id = ANY(site_ids);
  ...
  WHERE id = ANY(booking_ids) AND (
    ...
    OR (site_id IS NOT NULL AND site_id <> ALL(site_ids))
  );
  ...
  IF incoming_site_links > 0 OR incoming_opportunity_links > 0 OR incoming_previous_opportunity_links > 0
    OR incoming_lead_links > 0 OR incoming_communication_links > 0 THEN
  ...
  result := jsonb_build_object(
    ...
    'incomingReferences', incoming_site_links + incoming_opportunity_links + incoming_previous_opportunity_links + incoming_lead_links + incoming_communication_links,
