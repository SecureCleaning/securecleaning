-- Repair installations still running the obsolete compliance-current sale-start gate.
-- Reapply only sale creation, preserving newer handover/commission/payment-plan functions.
-- Approval and same-state/agent-ownership checks remain enforced in the database.
BEGIN;
