-- Apply after client_crm_optional_business_name_migration.sql.
-- Assigned agents may update their own shared client and site profile. Owners and
-- managers retain access to every opportunity. The RPC remains service-role only.
BEGIN;
...
BEGIN
  ...
  IF NULLIF(resolved_contact_name, '') IS NULL
  ...
  UPDATE crm_organisations SET
    business_name = BTRIM(COALESCE(p_business_name, '')), updated_at = NOW()
  WHERE id = organisation_row.id;

  UPDATE clients SET
    business_name = BTRIM(COALESCE(p_business_name, '')),
