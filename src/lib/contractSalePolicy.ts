...
export function buildContractSaleAgreement(input: {
  ...
  return [
    ...
    `${input.paymentPlanTerms ? 'Deposit required before inspection' : 'Deposit payable now'}: ${money(input.depositIncGstCents)} including GST`,
    ...
    input.paymentPlanTerms ? `3.1 Secure Cleaning will issue one tax invoice for the full purchase price. Payment dates and amounts follow Schedule 2 once accepted. At least ${money(input.depositIncGstCents)} including GST must clear before the site inspection.` : `3.1 Secure Cleaning will issue one tax invoice for the full purchase price. Only the deposit of ${money(input.depositIncGstCents)} including GST is due on receipt and must clear before the site inspection.`,
