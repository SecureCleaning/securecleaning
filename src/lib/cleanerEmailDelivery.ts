...
async function prepare(actor: AdminSessionIdentity, value: unknown) {
  ...
  const { data, error } = await db.from('cleaners').select('id,email,contact_name,first_name,last_name,business_name,city,suburb,state,status,broadcast_unsubscribe_token').in('email', input.emails)
  if (error) throw new Error('Cleaner lookup failed')
  const rows = data ?? []
  const [global, local] = await Promise.all([
    db.from('crm_email_suppressions').select('email_normalized').in('email_normalized', input.emails).eq('blocks_all', true),
    rows.length ? db.from('cleaner_broadcast_suppressions').select('cleaner_id').in('cleaner_id', rows.map(row => row.id)) : Promise.resolve({ data: [], error: null }),
  ])
  if (global.error || local.error) throw new Error('Email preferences could not be checked')
  const blockedEmails = new Set((global.data ?? []).map(row => row.email_normalized))
  const blockedIds = new Set((local.data ?? []).map(row => row.cleaner_id))
...
export async function previewCleanerEmail(actor: AdminSessionIdentity, value: unknown) {
  ...
  return { fingerprint: prepared.fingerprint, sender: prepared.sender, recipients: prepared.messages.map(message => ({ id: message.cleanerId, email: message.email, name: message.name, subject: message.subject, html: message.html })) }
}
async function results(actor: AdminSessionIdentity, requestId: string, duplicate: boolean) {
  ...
  const { data, error } = await db.from('cleaner_emails').select('id,to_email,subject,delivery_outcome').eq('batch_id', requestId).order('created_at')
  if (error) throw new Error('Delivery results unavailable')
  return { recipients: data ?? [], duplicate }
}
...
export async function deliverCleanerEmail(actor: AdminSessionIdentity, value: Record<string, unknown>) {
  ...
  const { data: reserved, error: reserveError } = await db.rpc('reserve_cleaner_email_batch', {
    p_id: requestId, p_actor_id: actor.id, p_input_hash: inputHash,
    p_messages: prepared.messages.map(message => ({ cleaner_id: message.cleanerId, email: message.email, subject: message.subject, body: message.body, body_html: prepared.input.bodyHtml, body_document: prepared.input.bodyDocument, html: message.html, text: message.text })),
  ...
  const { data: entries, error: entryError } = await db.from('cleaner_emails').select('id,cleaner_id').eq('batch_id', requestId)
  if (entryError) throw new Error('Unable to read reserved email records')
  for (const entry of entries ?? []) {
    const message = prepared.messages.find(candidate => candidate.cleanerId === entry.cleaner_id)!
    ...
    try {
      const response = await sendEmailOrThrow({
        from: process.env.FROM_EMAIL ?? 'quotes@securecleaning.com.au', to: message.email,
        replyTo: prepared.sender.email, subject: message.subject, html: message.html, text: message.text,
        headers: { 'List-Unsubscribe': `<${message.unsubscribeUrl.replace('/cleaner-email-preferences/', '/api/cleaner-email-preferences/')}>`, 'List-Unsubscribe-Post': 'List-Unsubscribe=One-Click' },
      })
      providerId = response && typeof response === 'object' && 'id' in response ? String(response.id) : null
    } catch (error) {
      // An ambiguous provider outcome is never automatically retried.
      outcome = error instanceof EmailProviderRejectedError && ['validation_error', 'missing_required_field', 'invalid_access', 'invalid_api_key', 'rate_limit_exceeded'].includes(error.providerErrorName || '') ? 'failed' : 'unknown'
    }
  ...
  return results(actor, requestId, false)
}
