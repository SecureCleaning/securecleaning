...
export default function AgentCleaners({ assigneeId, agentName, state, initialCleaners, initialTemplates, initialTotal }: Props) {
  ...
  const [messageDocument, setMessageDocument] = useState<Record<string, unknown> | null>(null)
  ...
  function chooseTemplate(id: string) {
    ...
    setTemplateId(id); setSubject(template?.subject ?? ''); setMessage(template?.body ?? ''); setMessageHtml(template?.body_html || plainTextToEmailHtml(template?.body ?? '')); setMessageDocument(template?.body_document ?? null); setEmailEditorKey((current) => current + 1); setEmailPreview(null)
  }
  ...
  async function openCleaner(cleanerId: string) {
    ...
    try {
      ...
      const detail = result as AgentCleanerDetail; setSelected(detail); setCreating(false); setForm(formOf(detail.cleaner)); setTab('details'); chooseTemplate(initialTemplates[0]?.id ?? '')
  ...
  function emailPayload() {
    return { templateId: templateId || null, subject, body: message, bodyHtml: messageHtml, bodyDocument: messageDocument }
  }
  async function previewEmail() {
    if (!selected || !subject.trim() || !hasRichEmailContent({ html: messageHtml, text: message })) return
    setBusy(true); setNotice(null)
    try { const response = await fetch(`${apiBase}/${selected.cleaner.id}/email`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'preview', ...emailPayload() }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Unable to preview email.'); setEmailPreview(result.preview) }
    catch (error) { setNotice({ type: 'error', message: error instanceof Error ? error.message : 'Unable to preview email.' }) } finally { setBusy(false) }
  }
  async function sendEmail() {
    if (!selected || !emailPreview?.previewFingerprint) return
    setBusy(true); setNotice(null)
    try { const response = await fetch(`${apiBase}/${selected.cleaner.id}/email`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...emailPayload(), previewFingerprint: emailPreview.previewFingerprint }) }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Unable to send email.'); setSelected({ ...selected, emails: [result.email as AgentCleanerEmailHistory, ...selected.emails] }); setEmailPreview(null); setNotice({ type: 'success', message: 'Email sent and logged.' }) }
  ...
  return <div className="space-y-6">
    ...
    {modalOpen ? <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-label="Cleaner record"><div className="mx-auto my-6 max-w-5xl overflow-hidden rounded-2xl bg-white shadow-xl"><div className="flex items-start justify-between border-b border-gray-200 p-5"><div><h2 className="text-xl font-bold text-[#1a2744]">{creating ? 'Add cleaner' : nameOf(selected!.cleaner)}</h2><p className="text-sm text-gray-600">{creating ? `New ${state} cleaner record` : selected!.cleaner.business_name}</p></div><button type="button" onClick={() => { setSelected(null); setCreating(false); setNotice(null) }} className="rounded-lg border border-gray-300 px-3 py-2 text-sm font-semibold">Close</button></div><div className="flex flex-wrap gap-2 border-b border-gray-200 px-5 py-3">{(['details', 'documents', 'comments', 'email'] as Tab[]).map((value) => <button key={value} type="button" disabled={creating && value !== 'details'} onClick={() => { setTab(value); setNotice(null) }} className={`rounded-full px-4 py-2 text-sm font-semibold disabled:opacity-40 ${tab === value ? 'bg-teal-700 text-white' : 'bg-gray-100 text-gray-700'}`}>{value[0].toUpperCase() + value.slice(1)}</button>)}</div>
      <div className="space-y-4 p-5">{noticeBox}{tab === 'details' ? <form onSubmit={saveCleaner} className="grid gap-4 sm:grid-cols-2">{field('businessName', 'Business name')}{field('abn', 'ABN')}{field('firstName', 'First name')}{field('lastName', 'Surname')}{field('email', 'Email', { type: 'email' })}{field('phone', 'Phone')}{field('alternatePhone', 'Alternate phone')}{field('address', 'Street address')}{field('suburb', 'Suburb')}{field('postcode', 'Postcode')}{field('city', 'City')}{field('state', 'State', { readOnly: true })}<label className="text-sm"><span className="mb-1 block font-medium text-gray-700">Status</span><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as CleanerStatus })} className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2">{statuses.map((status) => <option key={status} value={status}>{status.replaceAll('_', ' ')}</option>)}</select></label>{field('complianceStatus', 'Compliance status')}{field('services', 'Services (comma separated)', { wide: true })}{field('serviceAreas', 'Service areas (comma separated)', { wide: true })}{field('preferredWork', 'Preferred work', { wide: true })}{field('insuranceExpiry', 'Insurance expiry', { type: 'date' })}{field('policeCheckExpiry', 'Police check expiry', { type: 'date' })}{field('inductionExpiry', 'Induction expiry', { type: 'date' })}{field('internalOwner', 'Internal owner')}{field('rating', 'Internal rating', { type: 'number' })}<label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={form.workingWithChildrenCheck} onChange={(event) => setForm({ ...form, workingWithChildrenCheck: event.target.checked })} /> Working with children check</label><label className="text-sm sm:col-span-2"><span className="mb-1 block font-medium text-gray-700">Internal notes</span><textarea rows={4} value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="w-full rounded-lg border border-gray-300 p-3" /></label><div className="sm:col-span-2"><button disabled={busy} className="rounded-lg bg-teal-700 px-5 py-3 font-semibold text-white disabled:opacity-60">{busy ? 'Saving…' : 'Save cleaner'}</button></div></form> : null}
        ...
        {tab === 'email' && selected ? <div className="space-y-4"><p className="text-sm text-gray-600">To: <strong>{selected.cleaner.email}</strong>. Template edits apply only to this email.</p><label className="block text-sm font-semibold">Template<select value={templateId} onChange={(event) => chooseTemplate(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white p-3"><option value="">Custom email</option>{initialTemplates.map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label><label className="block text-sm font-semibold">Subject<input value={subject} onChange={(event) => { setSubject(event.target.value); setEmailPreview(null) }} maxLength={240} className="mt-1 w-full rounded-lg border border-gray-300 p-3" /><span className="mt-2 block"><EmailMergeFieldPicker fields={CLEANER_EMAIL_MERGE_FIELDS} onInsert={(token) => { setSubject((current) => appendEmailMergeField(current, token)); setEmailPreview(null) }} /></span></label><RichEmailEditor value={createRichEmailContent({ document: messageDocument, html: messageHtml, text: message })} resetKey={`agent-cleaner-email-${emailEditorKey}`} mergeFields={CLEANER_EMAIL_MERGE_FIELDS} onChange={(content) => { setMessage(content.text); setMessageHtml(content.html); setMessageDocument(content.document); setEmailPreview(null) }} /><div className="flex flex-wrap gap-3"><button type="button" disabled={busy || !subject.trim() || !hasRichEmailContent({ html: messageHtml, text: message })} onClick={() => void previewEmail()} className="rounded-lg bg-gray-900 px-4 py-2 font-semibold text-white disabled:opacity-60">{busy ? 'Working…' : 'Preview email'}</button><button type="button" disabled={busy || !emailPreview?.previewFingerprint} onClick={() => void sendEmail()} className="rounded-lg bg-teal-700 px-4 py-2 font-semibold text-white disabled:opacity-60">{busy ? 'Sending…' : 'Send email'}</button></div><p className="text-xs text-gray-500">Database fields are filled from the selected cleaner record during preview. Preview is required after every change. You receive a copy and replies go to your work email.</p><div className="divide-y divide-gray-100">{selected.emails.map((email) => <div key={email.id} className="py-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div><div className="font-semibold">{email.subject}</div><div className="text-xs text-gray-500">{dateOf(email.sent_at || email.created_at)} · {email.template_name || 'Custom email'} · {email.status}</div></div></div></div>)}{selected.emails.length === 0 ? <p className="py-3 text-sm text-gray-500">No emails sent yet.</p> : null}</div></div> : null}</div>
    </div></div> : null}
    <EmailPreviewModal open={Boolean(emailPreview)} title={emailPreview?.history ? 'Sent cleaner email' : 'Cleaner email preview'} subject={emailPreview?.subject ?? ''} from={emailPreview?.from} to={emailPreview?.to} cc={emailPreview?.cc} html={emailPreview?.html ?? ''} sending={busy} onClose={() => setEmailPreview(null)} onSend={emailPreview?.history ? undefined : () => void sendEmail()} />
  </div>
}
