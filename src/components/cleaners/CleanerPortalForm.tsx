...
export default function CleanerPortalForm({ mode, registeredEmail, initialProfile, lockedState }: { mode: 'update' | 'onboarding'; registeredEmail: string; initialProfile: CleanerPortalProfile | null; lockedState?: string }) {
  const [profile, setProfile] = useState(() => ({ ...(initialProfile ?? emptyProfile(registeredEmail)), ...(lockedState ? { state: lockedState } : {}) }))
