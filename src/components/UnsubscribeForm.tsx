...
export default function UnsubscribeForm({ token }: { token: string }) {
  ...
  if (status === 'done') {
    return <p role="status" className="rounded-xl border border-green-200 bg-green-50 p-4 text-green-800">You have been unsubscribed from Secure Cleaning marketing emails. Necessary quote or booking messages may still be sent when you request them.</p>
  }

  return (
    <div className="space-y-4">
      <p className="text-gray-700">Confirm that you no longer want to receive marketing emails from Secure Cleaning.</p>
