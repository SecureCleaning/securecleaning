...
export default function ContractSalesWorkspace({ portal = 'admin', assigneeId = '', initialProductId = '', initialSaleId = '', initialInvoiceId = '', initialTab = 'overview' }: {
  ...
  async function saveInvoiceTemplate() {
    ...
    try {
      ...
      setMessage('Invoice template saved for future invoices. For an existing invoice, use Apply saved bank details beside its preview.')
  ...
  return <div>
    ...
    {(data.actor.role === 'owner' || data.actor.role === 'manager') && invoiceTemplateDraft ? <section className="mb-5 rounded-2xl border border-gray-200 bg-white shadow-sm">
      ...
      {showInvoiceTemplate ? <div className="border-t border-gray-200 p-5">
        <p className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">Saving the template does not change issued invoices. The owner can apply saved bank details to an existing invoice below. The sending agent&apos;s name, title and email always come from their staff account.</p>
...
<div className="md:col-span-3"><h4 className="font-semibold">Bank payment details</h4><p className="text-sm text-gray-600">Saved for new invoices. To correct an existing invoice, the owner can apply these saved bank details beside its preview.</p></div>
    ...
    <div className="mb-5 grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
      <aside className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 max-h-[64vh] space-y-2 overflow-y-auto">
          {data.sales.map((item) => <button key={item.id} type="button" onClick={() => { setCreatingSale(false); setSelectedSaleId(item.id); setSelectedProductId(item.productId); setSaleDraft({ finalPrice: (item.agreedPurchasePriceIncGstCents / 100).toFixed(2), commencementDate: item.commencementDate, notes: item.notes }); setInspection((current) => ({ ...current, location: item.siteAddress })) }} className={`w-full rounded-xl border p-3 text-left ${selectedSaleId === item.id ? 'border-teal-500 bg-teal-50' : 'border-gray-200'}`}><strong className="block">{item.saleCode}</strong><span className="block text-sm">{item.productCode} · {item.cleanerBusiness}</span><span className={`mt-2 inline-flex rounded-full px-2 py-1 text-xs font-semibold ${statusTone(item.status)}`}>{item.status.replaceAll('_', ' ')}</span></button>)}
