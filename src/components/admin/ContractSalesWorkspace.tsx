...
export default function ContractSalesWorkspace({ portal = 'admin', assigneeId = '', initialProductId = '' }: {
  ...
  async function saveInvoiceTemplate() {
    ...
    try {
      ...
      setMessage('Invoice template saved. It will be snapshotted onto future invoices.')
  ...
  return <div>
    ...
    {(data.actor.role === 'owner' || data.actor.role === 'manager') && invoiceTemplateDraft ? <section className="mb-5 rounded-2xl border border-gray-200 bg-white shadow-sm">
      ...
      {showInvoiceTemplate ? <div className="border-t border-gray-200 p-5">
        <p className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">Issued invoices remain unchanged. The sending agent&apos;s name, title and email always come from their staff account.</p>
      ...
      <main className="min-w-0 space-y-5">
        {!sale ? <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          {tab === 'invoices' ? <section className="space-y-5"><div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-lg font-bold">Sale tax invoice</h3><p className="text-sm text-gray-600">Prepare one full Australian tax invoice for the confirmed purchase price. Only the $500 GST-inclusive deposit is payable immediately. Preparing does not email it; the Agreement tab sends both PDFs together.</p></div>{!primaryInvoice ? <button type="button" disabled={!sale.priceFinalised || Boolean(busy)} onClick={() => void run('invoice.issue', { idempotencyKey: crypto.randomUUID() }, 'Full tax invoice prepared. Create the agreement, then send both PDFs together.')} className="rounded-lg bg-teal-700 px-4 py-2 font-semibold text-white disabled:opacity-50">Prepare full tax invoice</button> : null}</div>{!primaryInvoice && !sale.priceFinalised ? <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Save the final GST-inclusive purchase price in Overview before preparing the invoice.</p> : null}<div className="mt-4 space-y-2">{sale.invoices.map((invoice) => <div key={invoice.id} className="grid gap-3 rounded-xl border border-gray-200 p-4 text-sm lg:grid-cols-[1fr_auto_auto_auto]"><div><strong>{invoice.invoiceNumber}</strong><p>{invoice.invoiceType === 'sale' ? 'Full sale tax invoice' : `Legacy ${invoice.invoiceType} invoice`} · {invoice.paymentTerms}</p><p className="text-gray-500">Delivery: {invoice.deliveryStatus}{invoice.deliveryStatus === 'pending' ? ' · prepared, not yet emailed' : ' · PDF attachment'}</p>{invoice.invoiceType === 'sale' ? <p className="mt-1 font-medium text-teal-800">Deposit required {money(invoice.depositRequiredIncGstCents)} · Remaining after deposit {money(invoice.totalIncGstCents - invoice.depositRequiredIncGstCents)}</p> : null}</div><div className="font-semibold">Total {money(invoice.totalIncGstCents)}<span className="block text-xs text-gray-500">Paid {money(invoice.paidCents)} · Outstanding {money(invoice.totalIncGstCents - invoice.paidCents)}</span></div><span className={`h-fit rounded-full px-2 py-1 text-xs font-semibold ${statusTone(invoice.status)}`}>{invoice.status}</span><div className="flex h-fit flex-wrap gap-2"><a href={`/api/admin/contract-sales/invoices?saleId=${sale.id}&invoiceId=${invoice.id}`} className="rounded-lg border border-gray-300 px-3 py-2 font-semibold">Download PDF</a>{invoice.deliveryStatus !== 'pending' ? <button type="button" onClick={() => void run('invoice.resend', { invoiceId: invoice.id }, 'Tax invoice email resent with its PDF attachment.')} className="rounded-lg border border-gray-300 px-3 py-2 font-semibold">Resend invoice only</button> : null}</div></div>)}</div></div>
            ...
            {primaryInvoice && primaryInvoice.status !== 'paid' && depositConfirmed && !sale.paymentPlan ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5"><h3 className="text-lg font-bold">Approve payment plan</h3><p className="text-sm">Agents, managers and owners may approve a monthly plan for the outstanding tax-invoice balance. A payment-plan agreement version must then be signed.</p><div className="mt-4 flex flex-wrap gap-3"><label className="text-sm font-medium">Instalments<input type="number" min="2" max="24" value={plan.count} onChange={(event) => setPlan({ ...plan, count: event.target.value })} className="ml-2 rounded-lg border border-gray-300 px-3 py-2" /></label><label className="text-sm font-medium">First due<input type="date" value={plan.firstDueOn} onChange={(event) => setPlan({ ...plan, firstDueOn: event.target.value })} className="ml-2 rounded-lg border border-gray-300 px-3 py-2" /></label><button type="button" onClick={() => void run('payment-plan.create', plan, 'Payment plan approved. Create a new payment-plan agreement version for signature.')} className="rounded-lg bg-amber-700 px-4 py-2 font-semibold text-white">Approve plan</button></div></div> : null}
