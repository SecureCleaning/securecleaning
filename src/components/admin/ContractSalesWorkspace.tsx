...
export default function ContractSalesWorkspace({ portal = 'admin', assigneeId = '', initialProductId = '' }: {
  ...
  return <div>
    <AdminPageHeader title="Product Sales" description="Review the sale, complete the agreement, issue the full tax invoice, collect the deposit, coordinate the inspection, and complete the cleaner handover." backHref={portal === 'agent' ? '/agent' : '/admin'} backLabel={portal === 'agent' ? 'Back to agent portal' : 'Back to overview'} />
      ...
      <main className="min-w-0 space-y-5">
        {!sale ? <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <section className="grid gap-2 sm:grid-cols-4 lg:grid-cols-7">{[
            ['Overview', true], ['Agreement', sale.agreement?.status === 'signed'], ['Tax invoice', Boolean(primaryInvoice)], ['Deposit', depositConfirmed], ['Inspection', sale.inspection?.status === 'completed'], ['Balance / plan', balanceComplete], ['Handover', Boolean(sale.handoverAt)],
          ...
          <div className="flex flex-wrap gap-2 border-b border-gray-200">{(['overview', 'agreement', 'invoices', 'inspection', 'activity'] as SaleTab[]).map((item) => <button key={item} type="button" onClick={() => setTab(item)} className={`px-4 py-3 text-sm font-semibold capitalize ${tab === item ? 'border-b-2 border-teal-700 text-teal-800' : 'text-gray-600'}`}>{item === 'invoices' ? 'Tax invoice & payments' : item}</button>)}</div>
