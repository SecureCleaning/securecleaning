...
export default function ContractSaleInspectionPanel({ sale, product, template, actorRole, onRefresh }: {
  ...
  const checklistFields = useMemo(() => [
    ...
    ['initialClean','Initial / spring clean','textarea'], ['accessHours','Access hours','textarea'], ['accessInstructions','Access instructions','textarea'],
  ...
  return <div className="space-y-5">
    ...
    <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <h3 className="text-lg font-bold">3. New site checklist</h3><p className="mt-1 text-sm text-gray-600">Review the pre-filled details, save, then print the condensed one-page A4 checklist. The full scope can be attached as following pages when required.</p>
      <div className="mt-4 grid gap-4 md:grid-cols-2">{checklistFields.map(([key,label,kind]) => <label key={key} className={`text-sm font-medium ${kind === 'textarea' ? 'md:col-span-2' : ''}`}>{label}{kind === 'textarea' ? <textarea rows={key === 'notes' ? 5 : 3} value={checklist[key]} onChange={(event) => setChecklist({ ...checklist, [key]: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5" /> : <input type={kind === 'date' ? 'date' : 'text'} value={checklist[key]} onChange={(event) => setChecklist({ ...checklist, [key]: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5" />}</label>)}</div>
      <label className="mt-4 flex items-start gap-3 rounded-xl border border-teal-200 bg-teal-50 p-4 text-sm text-teal-950"><input type="checkbox" checked={includeScope} onChange={(event) => setIncludeScope(event.target.checked)} className="mt-0.5 h-4 w-4" /><span><strong>Attach the full scope of works</strong><br />When selected, the one-page checklist prints first and the detailed room scope follows on separate pages.</span></label>
      <div className="mt-4 flex flex-wrap gap-3"><button type="button" onClick={() => void saveChecklist()} disabled={Boolean(busy)} className="rounded-lg bg-teal-700 px-4 py-2 font-semibold text-white disabled:opacity-50">Save checklist</button>{sale.checklist ? <a href={`/api/admin/contract-sales/checklists?saleId=${sale.id}&preview=1${includeScope ? '&includeScope=1' : ''}`} target="_blank" rel="noopener noreferrer" className="rounded-lg border border-gray-300 px-4 py-2 font-semibold">Print checklist PDF</a> : <span className="self-center text-sm text-gray-500">Save before printing.</span>}</div>
