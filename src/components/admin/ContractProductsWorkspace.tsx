...
function draftFromProduct(product: ContractProduct): ProductDraft {
  return {
    ...
    estimatedHoursPerVisit: product.estimatedHoursPerVisit === null ? '' : String(product.estimatedHoursPerVisit),
...
export default function ContractProductsWorkspace({ portal = 'admin', initialProductId = '', assigneeId = '' }: { portal?: 'admin' | 'agent'; initialProductId?: string; assigneeId?: string }) {
  ...
  async function save() {
    ...
    try {
      await action('product.update', { productId: selected.id, expectedUpdatedAt: selected.updatedAt, ...draft })
