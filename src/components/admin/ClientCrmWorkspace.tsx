...
import type { CrmAgentOption, CrmEmailTemplate, CrmLead } from '@/lib/clientCrmData'
...
const stageOptions = ['new', 'contacted', 'qualified', 'won', 'lost']
...
export default function ClientCrmWorkspace({ portal = 'admin' }: { portal?: 'admin' | 'agent' }) {
  ...
  async function loadWorkspace(preferredLeadId?: string) {
    ...
    const nextLeadId = preferredLeadId || selectedLeadId || next.leads[0]?.id || ''
  ...
  const filteredLeads = useMemo(() => {
    ...
    if (!query) return data?.leads ?? []
    return (data?.leads ?? []).filter((lead) => [lead.businessName, lead.contactName, lead.email, lead.postcode, lead.sourceProvider, lead.assignedStaffName].some((value) => value?.toLowerCase().includes(query)))
  }, [data?.leads, search])
  ...
  async function createLead(event: React.FormEvent) {
    ...
    setStatus(null)
    try {
      const result = await post({ action: 'lead.create', ...leadDraft })
      setLeadDraft(emptyLeadDraft())
      setView('pipeline')
      await loadWorkspace(result.result?.id)
      setStatus({ type: 'success', message: 'Lead created and assigned using the selected agent or postcode coverage.' })
    } catch (error) {
      setStatus({ type: 'error', message: error instanceof Error ? error.message : 'Unable to create the lead.' })
    } finally {
      setBusy('')
    }
  }
  ...
  async function updateLead() {
    ...
    setStatus(null)
    try {
      await post({ action: 'lead.update', id: selectedLead.id, ...leadEdit })
      await loadWorkspace(selectedLead.id)
      setStatus({ type: 'success', message: 'Lead workflow updated.' })
    } catch (error) {
      setStatus({ type: 'error', message: error instanceof Error ? error.message : 'Unable to update the lead.' })
    } finally {
      setBusy('')
    }
  }
  ...
  async function sendEmail() {
    ...
    try {
      ...
      await loadWorkspace(selectedLead.id)
      setStatus({ type: 'success', message: 'Email accepted by the provider and recorded in the lead history.' })
    } catch (error) {
  ...
  const hasUnresolvedEmail = selectedLead?.communications.some((item) => item.status === 'sending' || item.status === 'unknown') ?? false
  ...
  return (
    <div>
      <AdminPageHeader
        title="Client CRM"
        description={portal === 'agent' ? 'Manage assigned leads, client follow-ups, and email activity.' : 'Capture leads, assign regional agents, manage client outreach, and keep the first sales steps connected.'}
      ...
      {view === 'new' ? <form onSubmit={createLead} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <h2 className="text-lg font-bold text-gray-900">Create a lead</h2>
        <p className="mt-1 text-sm text-gray-600">The postcode suggests an agent automatically. Owners and managers can override that suggestion.</p>
        ...
        <button type="submit" disabled={busy === 'lead'} className="mt-4 rounded-lg bg-green-600 px-5 py-3 font-semibold text-white disabled:opacity-60">{busy === 'lead' ? 'Creating...' : 'Create lead'}</button>
      </form> : null}
