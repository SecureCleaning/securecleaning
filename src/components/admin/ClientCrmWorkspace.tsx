...
type WorkspaceData = {
  ...
  defaultCity: CrmServiceRegion | null
  actor: StaffAccount
}
...
function emptyLeadDraft(city: CrmServiceRegion | '' = '') {
  return {
    businessName: '', firstName: '', lastName: '', email: '', phone: '', address: '', suburb: '', postcode: '',
    city, sourceType: 'manual', sourceProvider: '', sourceReference: '',
...
function inspectionDateLabel(scheduledFor: string, endsAt: string, city: CrmOpportunity['city']) {
  const start = new Date(scheduledFor)
  const end = new Date(endsAt)
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime())) return scheduledFor
  ...
  return `${date}, ${start.toLocaleTimeString('en-AU', timeOptions)}-${end.toLocaleTimeString('en-AU', timeOptions)}`
}
...
export default function ClientCrmWorkspace({
  ...
  async function loadWorkspace(preferredLeadId?: string) {
    ...
    setData(next)
    setLeadDraft((current) => current.city ? current : { ...current, city: next.defaultCity ?? '' })
  ...
  async function createLead(event: React.FormEvent) {
    ...
    try {
      ...
      setLeadDraft(emptyLeadDraft(data?.defaultCity ?? ''))
      setView('pipeline')
    ...
    <div>
      ...
      {view === 'new' ? <form onSubmit={createLead} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <label className="text-sm font-medium text-gray-700">Business name (optional)<input value={leadDraft.businessName} onChange={(event) => setLeadDraft({ ...leadDraft, businessName: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5" /></label>
          ...
          <label className="text-sm font-medium text-gray-700">Last name (optional)<input maxLength={100} autoComplete="family-name" value={leadDraft.lastName} onChange={(event) => setLeadDraft({ ...leadDraft, lastName: event.target.value })} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5" /></label>
          ...
          <label className="text-sm font-medium text-gray-700">Service region<select required value={leadDraft.city} disabled={data.actor.role === 'agent'} onChange={(event) => setLeadDraft({ ...leadDraft, city: event.target.value as CrmServiceRegion, suburb: '', postcode: '' })} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5"><option value="">Select service region</option><option value="melbourne">Melbourne / VIC</option><option value="sydney">Sydney / NSW</option></select></label>
          <div className="md:col-span-2 lg:col-span-3"><LocalityAutocomplete key={leadDraft.city} city={leadDraft.city || undefined} suburb={leadDraft.suburb} postcode={leadDraft.postcode} onChange={(updates) => setLeadDraft({ ...leadDraft, suburb: updates.suburb, postcode: updates.postcode })} /></div>
        ...
        {selectedLead ? <div className="space-y-5">
          ...
          <section className={`rounded-xl border px-4 py-3 shadow-sm ${selectedLead.scheduledInspections.length > 0 ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-white'}`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-sm font-bold text-gray-900">Pending site inspection{selectedLead.scheduledInspections.length === 1 ? '' : 's'}</h2>
                {selectedLead.scheduledInspections.length === 0 ? <p className="mt-1 text-sm text-gray-500">No pending inspection appointment is currently scheduled.</p> : null}
              </div>
              {selectedLead.scheduledInspections.length > 0 ? <Link href={portal === 'agent' && data.actor.availabilityAssigneeId ? `/availability/quoters/${encodeURIComponent(data.actor.availabilityAssigneeId)}` : '/admin/calendar'} className="text-sm font-semibold text-blue-700 hover:underline">View calendar</Link> : null}
            </div>
            {selectedLead.scheduledInspections.length > 0 ? <div className="mt-2 divide-y divide-blue-200">{selectedLead.scheduledInspections.map((inspection) => <div key={inspection.bookingRef} className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 py-2 text-sm"><span className="font-semibold text-blue-950">{inspectionDateLabel(inspection.scheduledFor, inspection.endsAt, selectedLead.city)}</span><span className="text-blue-800">{inspection.assignedAgentName ?? selectedLead.assignedStaffName ?? 'Agent to be confirmed'} · {inspection.bookingRef}</span></div>)}</div> : null}
          </section>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              {data.actor.role === 'owner' ? <DeleteClientCrmButton opportunityId={selectedLead.id} onDeleted={() => { setSelectedLeadId(''); void loadWorkspace() }} /> : null}
            </div>
