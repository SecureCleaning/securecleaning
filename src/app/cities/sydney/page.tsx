    ...
    <div className="min-h-screen">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              <div className="bg-white rounded-2xl border border-gray-100 p-8">
                ...
                <p className="text-gray-600 leading-relaxed">
                  {getContentValue(content, 'sydney.why_body_2', "With Sydney's high cost of doing business, reliability is everything. Our Owner-Operators have invested in their territories — they have every incentive to show up, do a great job, and keep your business.")}
                </p>
              </div>

              <div className="bg-white rounded-2xl border border-gray-100 p-8">
                <h2 className="text-2xl font-bold mb-5" style={{ color: '#1a2744' }}>
                  {getContentValue(content, 'sydney.areas_title', 'Areas We Cover')}
                </h2>
                ...
                <p className="mt-4 text-sm text-gray-500">
                  {getContentValue(content, 'sydney.areas_note', "Don't see your suburb? We likely cover it — get a quote to confirm.")}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                <h3 className="font-bold mb-4" style={{ color: '#1a2744' }}>{getContentValue(content, 'sydney.pricing_title', 'Sydney Pricing')}</h3>
                <p className="text-sm text-gray-600 mb-4">
                  {getContentValue(content, 'sydney.pricing_body', 'Sydney pricing reflects local labour market conditions. Use our instant quote calculator for an accurate, transparent estimate.')}
                </p>
                <Link href="/quote?city=sydney"
                  className="block text-center px-5 py-3 rounded-xl font-bold text-white transition-all"
                  style={{ backgroundColor: '#22c55e' }}>
                  {getContentValue(content, 'sydney.pricing_cta_label', 'Get Sydney Quote')}
                </Link>
              </div>

              <div className="bg-white rounded-2xl border border-gray-100 p-6">
                <h3 className="font-bold mb-3" style={{ color: '#1a2744' }}>{getContentValue(content, 'sydney.services_title', 'Services Available')}</h3>
      ...
      <section className="py-12 text-white text-center" style={{ backgroundColor: '#22c55e' }}>
        <div className="max-w-2xl mx-auto px-4">
          <h2 className="text-3xl font-bold mb-4">{getContentValue(content, 'sydney.bottom_cta_title', 'Ready to get started in Sydney?')}</h2>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/quote?city=sydney"
              ...
              {getContentValue(content, 'sydney.bottom_cta_primary_label', 'Get Sydney Quote')}
            </Link>
            <Link href="/booking"
