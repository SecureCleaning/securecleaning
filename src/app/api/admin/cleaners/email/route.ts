...
import { continueCleanerEmail, cleanerEmailResults, deliverCleanerEmail, previewCleanerEmail } from '@/lib/cleanerEmailDelivery'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (input.action === 'continue') return NextResponse.json({ success: true, result: await continueCleanerEmail(actor, input.requestId) })
