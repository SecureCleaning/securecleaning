  ...
  return <main className="min-h-screen bg-slate-100 px-4 py-8"><div className="mx-auto max-w-4xl">
    ...
    <article className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <section className="mt-7"><h2 className="text-xl font-bold">Scope overview</h2><p className="mt-2 text-gray-600">{job.cleanerScopeSnapshot.summary}</p>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">{job.cleanerScopeSnapshot.rooms.map((room, index) => <div key={`${room.type}-${index}`} className="rounded-xl border border-gray-200 p-4"><h3 className="font-bold">{room.label} · Qty {room.quantity}</h3><p className="mt-1 text-xs text-gray-500">{room.size > 0 ? `${room.size} sqm each · ` : ''}Floor {room.floor}</p>{room.description ? <p className="mt-2 text-sm text-gray-600">{room.description}</p> : null}<ul className="mt-2 space-y-1 text-sm text-gray-600">{room.tasks.map((task, taskIndex) => { const display = taskDisplay(task); return <li key={`${display.label}-${taskIndex}`} className="flex justify-between gap-3"><span>{display.label}</span><strong className="shrink-0 text-xs">{display.cadence}</strong></li> })}</ul>{(room.selectedOptions ?? []).length > 0 ? <div className="mt-3 border-t border-gray-100 pt-2 text-xs text-gray-600"><strong className="uppercase text-gray-500">Selected for this area</strong><ul className="mt-1 list-disc pl-4">{room.selectedOptions?.map((option) => <li key={option}>{option}</li>)}</ul></div> : null}</div>)}</div>
      </section>
