...
test('CRM can create a staff-selected inspection appointment outside public availability', () => {
  ...
  assert.match(workspace, /Duration \(minutes\)/)
  assert.match(workspace, /Inspection agent/)
  assert.match(workspace, /not limited by public inspection zones or preset availability/)
  assert.doesNotMatch(workspace, /availabilityFor: selectedLead\.id/)
  ...
  assert.match(appointments, /getManualInspectionWindow/)
  assert.doesNotMatch(appointments, /getAvailabilityCalendar/)
  assert.match(appointments, /availabilityAssignee\.city !== city/)
