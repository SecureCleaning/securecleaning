...
test('workbench keeps connected records together and exposes the complete gated workflow', () => {
  ...
  assert.match(workspace, /Search product sales/)
  assert.match(workspace, /Sale, client or cleaner/)
  assert.match(workspace, /filteredSales\.map/)
  for (const field of ['item.saleCode', 'item.productCode', 'item.clientBusiness', 'item.clientName', 'item.clientEmail', 'item.cleanerName', 'item.cleanerBusiness', 'item.suburb', 'item.state']) assert.match(workspace, new RegExp(field.replace('.', '\\.')))
  assert.match(workspace, /Client:<\/span> \{item\.clientBusiness \|\| item\.clientName/)
  assert.match(workspace, /Cleaner:<\/span> \{item\.cleanerBusiness/)
  assert.match(workspace, /No product sales match your search/)
  assert.match(workspace, /Client business/)
