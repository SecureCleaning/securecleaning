...
test('workbench keeps connected records together and exposes the complete gated workflow', () => {
  ...
  for (const field of ['item.saleCode', 'item.productCode', 'item.clientName', 'item.clientEmail', 'item.cleanerName', 'item.cleanerBusiness', 'item.suburb', 'item.state']) assert.match(workspace, new RegExp(field.replace('.', '\\.')))
  assert.match(workspace, /Client:<\/span> \{item\.clientName/)
  assert.match(workspace, /Cleaner:<\/span> \{item\.cleanerBusiness/)
  assert.match(workspace, /No product sales match your search/)
  assert.match(workspace, /\{sale\.clientName \|\| 'Not provided'\}/)
