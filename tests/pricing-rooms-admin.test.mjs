...
test('room tasks, mopping, and quote extras expose cadence controls and scope output', () => {
  ...
  assert.match(roomAdmin, /Default charges:/)
  assert.match(roomAdmin, /minutes per sqm replaces generic area labour/)
  assert.match(roomAdmin, /Minutes \/ sqm/)
  assert.match(roomAdmin, /hourly labour rate/)
  ...
  assert.match(roomAdmin, /Add this fixed charge on top of task-based floor pricing/)
