...
test('cleaner directory uses a signed token bootstrap, no indexing, server-side access, and tracked interest delivery', () => {
  ...
  const interest = source('src/lib/contractProductInterest.ts')
  const products = source('src/lib/contractProducts.ts')
  ...
  assert.doesNotMatch(products.match(/export type CleanerJobListing[\s\S]+?\n>/)?.[0] ?? '', /sourceQuoteRef|opportunityId|clientPricePerVisit|clientDisplayName/)
  assert.match(products, /clientDisplayName: string/)
  assert.match(products, /crm_organisations'[\s\S]+business_name, legal_name/)
  assert.match(products, /clients'[\s\S]+business_name, contact_name/)
  assert.match(workspace, /product\.clientDisplayName \|\| 'Client not provided'/)
  assert.match(workspace, /Private client/)
  assert.match(workspace, /selected\.clientDisplayName \|\| 'Not provided'/)
  assert.doesNotMatch(products.match(/export async function getAvailableCleanerJobs[\s\S]+?\n}/)?.[0] ?? '', /clientDisplayName/)
  assert.doesNotMatch(broadcasts.match(/return products\.map<BroadcastProduct>[\s\S]+?\n  \}\)\)/)?.[0] ?? '', /clientDisplayName/)
  assert.match(products, /assertCleanerListingExcludesSourcePii/)
  ...
  assert.match(interestRoute, /status: 503/)
  assert.match(interest, /accessLink\.state && accessLink\.state !== productRow\.state/)
  assert.match(interest, /\.eq\('email', email\)\.maybeSingle\(\)/)
  assert.doesNotMatch(interest, /\.ilike\('email', email\)/)
  assert.match(interest, /match_status: cleaner \? 'approved_cleaner' : 'unmatched'/)
  assert.match(interest, /audience: 'agent'/)
  assert.match(interest, /No job details or confirmation email were sent to this address/)
  assert.match(interest, /if \(input\.cleaner\)/)
  assert.match(interest, /contract_product_interest_notifications/)
  assert.match(interest, /contract_product_activity/)
  assert.match(interest, /event_type: 'interest_registered'/)
  assert.match(interest, /notification_status: agentNotification\?\.status \?\? 'pending'/)
  assert.match(interest, /providerAccepted \|\| !definitelyRejected \? 'unknown' : 'failed'/)
  const notificationMigration = source('supabase/contract_products_interest_notifications_migration.sql')
  assert.match(notificationMigration, /ALTER COLUMN cleaner_id DROP NOT NULL/)
  assert.match(notificationMigration, /UNIQUE\(interest_id, audience\)/)
  assert.match(notificationMigration, /'unknown'/)
  assert.match(notificationMigration, /ENABLE ROW LEVEL SECURITY/)
  const activityMigration = source('supabase/contract_product_activity_migration.sql')
  assert.match(activityMigration, /CREATE TABLE IF NOT EXISTS contract_product_activity/)
  assert.match(activityMigration, /REVOKE ALL ON TABLE contract_product_activity FROM PUBLIC, anon, authenticated/)
  assert.match(activityMigration, /GRANT ALL ON TABLE contract_product_activity TO service_role/)
  assert.match(activityMigration, /INSERT INTO contract_product_activity[\s\S]+contract_product_interests/)
  assert.match(activityMigration, /sync_contract_product_activity_interest_status/)
  assert.match(products, /contract_product_activity/)
  assert.match(workspace, /Product activity/)
  assert.match(workspace, /These details are not included in public listings or broadcasts/)
  assert.match(workspace, /mailto:\$\{item\.email\}/)
  assert.match(workspace, /tel:\$\{item\.phone\}/)
...
test('broadcasts are state-scoped, suppressed, idempotent, individually addressed, and unsubscribable', () => {
  const broadcasts = source('src/lib/contractProductBroadcasts.ts')
  const migration = source('supabase/contract_products_migration.sql')
  const targetingMigration = source('supabase/contract_product_broadcast_templates_and_targeting_migration.sql')
  const multipleRecipientsMigration = source('supabase/contract_product_broadcast_multiple_recipients_migration.sql')
  ...
  const workspace = source('src/components/admin/ContractProductsWorkspace.tsx')
  assert.match(workspace, /sessionStorage\.setItem\('cleaner-broadcast-request', requestId\)/)
  assert.match(workspace, /if \(!result\.inProgress\)[\s\S]+setBroadcastRequestId\(''\)/)
  assert.match(broadcasts, /to: cleaner\.email/)
  assert.doesNotMatch(broadcasts, /selectedRecipients\.length > 50/)
  assert.match(broadcasts, /create_cleaner_broadcast_campaign_v3/)
  assert.match(broadcasts, /Select an eligible approved cleaner in this state/)
  assert.match(broadcasts, /type BroadcastRecipientMode = 'state' \| 'single' \| 'multiple'/)
  assert.match(broadcasts, /split\(\/\[,;\\n\]\+\//)
  assert.match(broadcasts, /do not match eligible approved cleaners in this state/)
  assert.match(broadcasts, /different cleaner selection/)
  ...
  assert.match(targetingMigration, /recipient_mode TEXT NOT NULL DEFAULT 'state'/)
  assert.match(targetingMigration, /p_recipient_cleaner_ids UUID\[\]/)
  assert.match(targetingMigration, /c\.id = ANY\(p_recipient_cleaner_ids\)[\s\S]+c\.status = 'approved' AND c\.state = p_state/)
  assert.match(targetingMigration, /eligible_count <> expected_recipients/)
  assert.match(multipleRecipientsMigration, /recipient_mode IN \('state', 'single', 'multiple'\)/)
  assert.match(multipleRecipientsMigration, /p_recipient_mode = 'multiple' AND expected_recipients < 2/)
  assert.match(multipleRecipientsMigration, /cleaner_broadcast_recipients[\s\S]+cleaner_id <> ALL\(p_recipient_cleaner_ids\)/)
  assert.match(multipleRecipientsMigration, /created_by_staff_id UUID REFERENCES admin_staff_accounts/)
  assert.match(multipleRecipientsMigration, /p_actor_role <> 'owner' AND p_sender_staff_id IS DISTINCT FROM p_actor_id/)
  assert.match(multipleRecipientsMigration, /REVOKE ALL ON FUNCTION create_cleaner_broadcast_campaign_v3[\s\S]+FROM PUBLIC, anon, authenticated/)
  assert.match(broadcasts, /Only the owner can send a product broadcast as another team member/)
  assert.match(broadcasts, /from: `\$\{safeHeaderName\(deliverySender\.displayName\)\} - Secure Cleaning/)
  assert.match(broadcasts, /replyTo: deliverySender\.email/)
  assert.match(unsubscribe, /unsubscribeCleanerBroadcast/)
})

test('broadcast templates are owner-managed while agents can edit a copied email draft', () => {
  const templates = source('src/lib/contractProductBroadcastTemplates.ts')
  const route = source('src/app/api/admin/contract-products/route.ts')
  const workspace = source('src/components/admin/ContractProductsWorkspace.tsx')
  const migration = source('supabase/contract_product_broadcast_templates_and_targeting_migration.sql')
  assert.match(templates, /actor\.role !== 'owner'/)
  assert.match(templates, /Only the owner can save broadcast templates/)
  assert.match(route, /broadcast\.template\.save/)
  assert.match(route, /broadcast\.recipients/)
  assert.match(route, /listContractProductBroadcastSenders\(actor\)/)
  assert.match(workspace, /You can edit it for this send/)
  assert.match(workspace, /One cleaner/)
  assert.match(workspace, /Selected cleaners/)
  assert.match(workspace, /cleanerEmails: broadcastCleanerEmails/)
  assert.match(workspace, /Enter two or more addresses separated by commas, semicolons, or new lines/)
  assert.match(workspace, /Unless the owner changes this selection, it defaults to whoever is signed in/)
  assert.match(workspace, /data\.actor\.role === 'owner'/)
  assert.match(workspace, /senderStaffId: broadcastSenderId/)
  assert.match(workspace, /Sent as \{campaign\.senderName/)
  assert.match(workspace, /Automatic jobs link for/)
  assert.match(migration, /CREATE TABLE IF NOT EXISTS cleaner_broadcast_templates/)
  assert.match(migration, /Service role full access - cleaner_broadcast_templates/)
})
