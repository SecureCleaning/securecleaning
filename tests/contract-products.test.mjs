...
test('broadcasts are state-scoped, suppressed, idempotent, individually addressed, and unsubscribable', () => {
  ...
  assert.match(broadcasts, /selectedRecipients\.length > 50/)
  ...
  assert.match(broadcasts, /from: `\$\{safeHeaderName\(sender\.displayName\)\} - Secure Cleaning/)
  assert.match(broadcasts, /replyTo: sender\.email/)
...
test('broadcast templates are owner-managed while agents can edit a copied email draft', () => {
  ...
  assert.match(workspace, /Enter 2–50 addresses separated by commas, semicolons, or new lines/)
